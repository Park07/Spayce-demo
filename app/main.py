...
def _sidebar_controls() -> dict[str, Any]:
    ...
    max_capacity = st.sidebar.slider("Zone capacity limit", min_value=3, max_value=100, value=10, step=1,
    ...
    with st.sidebar.expander("Advanced", expanded=False):
        confidence = st.slider("Detection confidence", min_value=0.01, max_value=0.99, value=0.35, step=0.05)
        sample_stride = st.slider("Frame stride", min_value=1, max_value=15, value=5, step=1)
...
def main() -> None:
    ...
    # ── Row 2: Video + Actions ──
    vid_col, action_col = st.columns([3, 1], gap="large")
