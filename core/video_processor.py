...
def process_video_file(
    ...
    bimodal_predictor = BiModalPredictor(zone_names=zone_names, zone_types=zone_types, critical_density_per_zone=max_capacity)
    ...
    while True:
        ...
        # === Frame-by-frame CSV logging for prediction validation ===
        import csv, os
        from datetime import datetime as _dt
        log_dir = os.path.join(str(root_dir), "outputs", "logs", f"{input_path.stem}_{session_id}")
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, "frame_log.csv")
        write_header = not os.path.exists(log_path) or frame_idx <= 1
        with open(log_path, "a" if not write_header else "w", newline="") as f:
            csv_w = csv.writer(f)
            if write_header:
                csv_w.writerow([
        ...
        # --- Operator HUD ---
        danger_zones = [zm["zone"] for zm in zone_metrics if zm["count"] >= max_capacity - 1]
...
def init_live_state(
    ...
    return {
        ...
        "latest_zone_metrics": [],
        "bimodal_predictor": BiModalPredictor(zone_names=zone_names, zone_types=zone_types, critical_density_per_zone=controls.get("max_capacity", 10)),
        }
...
def process_live_frame(
    ...
    danger_zones = [zm["zone"] for zm in zone_metrics if zm["count"] >= mc - 1]
